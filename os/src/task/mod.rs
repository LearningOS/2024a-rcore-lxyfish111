...
impl TaskManager {
    ...
    fn mark_current_exited(&self) {
        ...
        let tm = get_time_ms();
        let start_tm = inner.tasks[current].time;
        inner.tasks[current].time = tm - start_tm;
    }
...
pub fn increase_current_task_syscall_count(syscall_id: usize) {
    ...
    let pos = inner.current_task;
    let cnt = inner.tasks[pos].syscall_times[syscall_id].borrow_mut();
    *cnt += 1;
}
